...
4. Suppose $F$ is any anti-derivative on $[a,b]$
	...
	5. By [[Mean Value Theorem|MVT]], $\exists c \in (x_{i-1},x_{i})$ s.t $f'(c) = \frac{f(x_{i})-f(x_{i-1})}{x_{i}-x_{i-1}}$
	6. $\implies f'(c) = f(x_{i})-f(x_{i-1}) (\triangle x)$ by defn of $\triangle x$
	7. Note that $f'(c) = F(c) = F(x)|_{b}^{a}$
	...
	12. This is a [[Telescoping Series]], then, you should get after computation: $\lim_{ n \to \infty }F(x_{0}) - F(x_{n}) = \lim_{ n \to \infty } F_{a} - F_{B}$
	13. Thus, $\int_{a}^{b} f(x) \, dx = F_{a} - F_{b}$