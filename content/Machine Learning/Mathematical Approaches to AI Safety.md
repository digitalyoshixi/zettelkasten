...
- Notations will be:
	- $\mathcal{A},\mathcal{O},\mathcal{R}$ are the finite sets of actions, observations and rewards repectively
	- $\epsilon := \mathcal{O} \times \mathcal{R}$ to denote set of percepts (additionally $e_{t} = o_{t}r_{t}$)
	- $\mathcal{H} :=(A \times \epsilon)^{*}$ denotes set of finite interaction histories
